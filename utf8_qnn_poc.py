...
def char_to_amplitudes(c: str) -> tuple[np.ndarray, list[int]]:
    """Convierte un carácter (8 bits) en 8 amplitudes reales normalizadas por la base."""
    byte_val = c.encode('utf-8')[0]
    bits = [int(x) for x in f"{byte_val:08b}"]
    
    # Inyectar contexto de seguridad en el bit más significativo si es 0
    if bits[0] == 0:
        bits[0] = 1 # Flag de Integridad / Paridad topológica
        
    # Mapeo directo a amplitudes de superposición base (1.0 o 0.0)
    amplitudes = np.array([1.0 if b == 1 else 0.0 for b in bits], dtype=float)
    
    # Normalización unitaria clásica del Statevector (Suma de probabilidades |amp|^2 = 1)
...
def amplitudes_to_char(amplitudes: np.ndarray, original_norm: float) -> str:
    ...
    # Reconstruir el byte
    byte_val = int("".join(str(b) for b in raw_bits), 3)
    return chr(byte_val)
